Raphael.fn.pieChart = function (cx, cy, r, values, colors) {
	...
	var teamNum = -1;
	var start = function () {
				for(j=0; j<top_opt.length; j++){
					top_opt[j].stop().animate({transform: "s1.2 1.2 " + cx + " " + cy}, ms, "elastic");
                	opt[j].stop().animate({transform: "s1.2 1.2 " + cx + " " + cy}, ms, "elastic");
				}
			},
			move = function (dx, dy) {
			   angle = Math.atan2 (dy, dx) * 180/Math.PI;
			   var eachAngle = 90/top_opt.length;
			   var k = -1;
			   if (angle <= eachAngle) {
				   k = 0;
			   } else if (angle > eachAngle && angle <= eachAngle*2) {
				   k = 1;
			   } else if (angle > eachAngle*2 && angle <=eachAngle*3) {
				   k = 2;
			   } else if (angle > eachAngle*3 && angle <=eachAngle*4) {
				   k = 3;
			   } else if (angle > eachAngle*4 ) {
				   k = 4;
			   } 
			   if (k != -1 ){
				   switch(k){
					...
					top_opt.toFront();
					teamNum = k;
				   for(l=0; l<top_opt.length; l++){
					   if( l != k ) {
							top_opt[l].stop().animate({transform: ""}, 0, "elastic");
							opt[l].stop().animate({transform: ""}, 0, "elastic");
					   } else {
							top_opt[l].stop().animate({transform: "s1.5 1.5 " + cx + " " + cy}, 0, "elastic");
				  			opt[l].stop().animate({transform: "s1.5 1.5 " + cx + " " + cy}, 0, "elastic");
					   }
				   }
			   }
			},
			up = function () {
				j = teamNum;
				top_opt[j].stop().animate({transform: ""}, 0, "elastic");
				opt[j].stop().animate({transform: ""}, 0, "elastic");
				changeTeam(j);
				mask.remove();
				mask = opt[j].clone();
